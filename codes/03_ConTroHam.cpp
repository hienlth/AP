...
void SapXep(int **&a, int m, int n, 
		bool(*pSort)(int, int));
bool TangDan(int X, int Y);
bool GiamDan(int X, int Y);
bool Chan_Dau_Day(int X, int Y);
...
int main()
{
	...
	SapXep(A, M, N, Chan_Dau_Day);
	cout << "Chan dau day: " << endl;
	XuatMang(A, M, N);
	system("pause");
	return 0;
}
...
bool Chan_Dau_Day(int X, int Y)
{
	//Y chẵn, X lẻ đổi chổ X, Y
	if ((Y % 2 == 0) && (X % 2 == 1)) return true;
	//ngược lại Y lẻ hoặc X chẵn
